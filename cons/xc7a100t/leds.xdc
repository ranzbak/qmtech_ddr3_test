# 
# Paul Honig 2020
#
# I/O Board
# Open AARS board V2
#
# Core board
# QMTech Artix-7XC7A100T Core Board

# LEDS
set_property PACKAGE_PIN U21 [get_ports {o_leds[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports o_leds[0]]
set_property PACKAGE_PIN V23 [get_ports {o_leds[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports o_leds[1]]
set_property PACKAGE_PIN Y22 [get_ports {o_leds[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports o_leds[2]]
set_property PACKAGE_PIN Y25 [get_ports {o_leds[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports o_leds[3]]
set_property PACKAGE_PIN W21 [get_ports {o_leds[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports o_leds[4]]
set_property PACKAGE_PIN Y21 [get_ports {o_leds[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports o_leds[5]]
set_property PACKAGE_PIN AB24 [get_ports {o_leds[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports o_leds[6]]
set_property PACKAGE_PIN AC24 [get_ports {o_leds[7]}]
set_property IOSTANDARD LVCMOS33 [get_ports o_leds[7]]

